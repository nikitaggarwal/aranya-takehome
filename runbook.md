...
Apply the official manifests into a new `argocd` namespace:
...
## 12. Encrypt the admin kubeconfig for Christian, Yoofi, and Sasi
...
Encrypt the kubeconfig for all three recipients in one file. The `--trust-model always` flag tells GPG to encrypt to these keys even though we haven't manually signed them as trusted — appropriate here since the keys came from a trusted out-of-band source (the same email thread that gave us the take-home).

```bash
gpg --trust-model always \
    --output kubeconfig.gpg \
    --encrypt \
...
```bash
gpg --decrypt kubeconfig.gpg > admin.kubeconfig
```

Attach `kubeconfig.gpg` to your reply email along with `runbook.md`.
...
These are the two non-obvious problems I hit on the original build. Both ended up encoded into the inventory in this repo, so a fresh run of this runbook shouldn't re-trigger them — but they're worth knowing.
